...
class CSFDMovieCache:
	...
	def getMoviesFromCache(self, channel):
		...
		if self.cacheMovieList.has_key(channel) and self.cacheMovieListValidity.has_key(channel):
	...
	def delChannelFromMovieCache(self, channel):
		...
		if channel is not None and channel != '':
			if self.cacheMovieList.has_key(channel):
				del self.cacheMovieList[channel]
			if self.cacheMovieListValidity.has_key(channel):
